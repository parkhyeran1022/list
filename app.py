...
agency_data = [
    ...
    {"소속": "트레져헌터", "플랫폼": "유튜브", "이름": "버터와여름이네", "구독자": "22.96만", "단가": "RS 30%", "비고": "솔브 전달완료"}
]
...
def get_yt_profile_pic(url, api_key):
    ...
    if not api_key: return "https://via.placeholder.com/300x300.png?text=Glowuprizz"
    try:
    ...
    if st.button("💾 구글 시트에 컨펌 결과 저장하기"):
        if conn:
            try:
                ...
                st.success("✅ 저장 완료!")
            except: st.error("구글 시트 저장 권한을 확인하세요.")
