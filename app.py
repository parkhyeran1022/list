...
# API 및 구글 시트 연결 (Secrets 사용)
yt_key = st.secrets.get("YOUTUBE_KEY", "")
try:
    conn = st.connection("gsheets", type=GSheetsConnection)
except Exception as e:
    st.error("⚠️ 구글 시트 연결 설정이 필요합니다. (Secrets 확인)")
    conn = None
...
yt_data = [
    ...
    # 🏢 자사 - 파트너십 (5명)
    {"구분": "자사", "세부유형": "파트너십", "이름": "매드브로 MadBros", "URL": "https://youtube.com/channel/UCiTcv_AxQQSx77yGikHHDZw", "추천제품": "볼캡 쏙쉐이크", "아이디어": ""},
    ...
    {"구분": "자사", "세부유형": "파트너십", "이름": "kiu기우쌤", "URL": "https://youtube.com/channel/UCIZ5rCTYJ0s16FgT7OetVEQ", "추천제품": "솔브 모델링팩 괄사크림", "아이디어": ""},
    ...
    # 🌍 외부 - 외부 (40명)
    {"구분": "외부", "세부유형": "외부", "이름": "조재원", "URL": "https://youtube.com/channel/UC2o_y872S6YvaO1K8EYnoxg", "추천제품": "쏙쉐이크", "아이디어": "동금여사님과 얼먹 먹방"},
    {"구분": "외부", "세부유형": "외부", "이름": "송대익", "URL": "https://youtube.com/channel/UCreFV1bKkKE6ufPtd5XeEJw", "추천제품": "쏙쉐이크", "아이디어": "자취 생활 노출"},
    {"구분": "외부", "세부유형": "외부", "이름": "엄지렐라 Umjirella", "URL": "https://youtube.com/channel/UCLXafJ8yYXeUN_eHai-6Pgw", "추천제품": "어퍼 볼캡", "아이디어": ""},
    {"구분": "외부", "세부유형": "외부", "이름": "숏박스", "URL": "https://youtube.com/channel/UC1B6SalAoiJD7eHfMUA9QrA", "추천제품": "어퍼 체크셔츠", "아이디어": ""},
    {"구분": "외부", "세부유형": "외부", "이름": "김선태", "URL": "https://youtube.com/channel/UCt-BApVtJGrvF5pCgbiNVeg", "추천제품": "어퍼 볼캡", "아이디어": ""},
    ...
    {"구분": "외부", "세부유형": "외부", "이름": "고기남자", "URL": "https://youtube.com/channel/UCT3CumbFIJiW33uq0UI3zlg", "추천제품": "쏙쉐이크", "아이디어": "요리 먹방 관리"},
    ...
    {"구분": "외부", "세부유형": "외부", "이름": "Jiwon 지원", "URL": "https://youtube.com/channel/UCURnLWTrLZVv_KiVaeZakog", "추천제품": "멜브 솔브", "아이디어": ""},
    ...
    {"구분": "외부", "세부유형": "외부", "이름": "보현Bohyun", "URL": "https://youtube.com/channel/UCZkNETnsbFXG8h7RPQyBtNw", "추천제품": "멜브 솔브", "아이디어": ""},
    ...
    {"구분": "외부", "세부유형": "외부", "이름": "이리유", "URL": "https://youtube.com/channel/UCRFh2MoRiD-whPn0QuOzMFQ", "추천제품": "멜브 솔브", "아이디어": ""},
    {"구분": "외부", "세부유형": "외부", "이름": "기몌민", "URL": "https://youtube.com/channel/UC78welXHRpNJJfiB2zg5BSQ", "추천제품": "멜브 솔브", "아이디어": ""},
...
df_yt_master = pd.DataFrame(yt_data)
...
df_vendor_master = pd.DataFrame(vendor_data)
...
agency_data = [
    ...
    {"소속": "샌드박스", "플랫폼": "쇼츠", "이름": "송타민", "구독자": "20,800", "단가": "910,000", "비고": "RS 30%"},
    {"소속": "트레져헌터", "플랫폼": "유튜브 커머스", "이름": "버터와여름이네", "구독자": "22.96만명", "단가": "RS 30%", "비고": "솔브 전달완료"}
]
...
def get_yt_profile_pic(url, api_key):
    ...
    if not api_key: return "https://via.placeholder.com/300x300.png?text=No+Key"
    try:
    ...
    for i, row in df_subset.iterrows():
        with cols[i % 4]:
            with st.container(border=True):
                ...
                st.subheader(row['이름'])
                st.caption(f"{row.get('세부유형', row.get('분류', ''))}")
                st.markdown(f"**🎯 추천:** {row.get('추천제품', '-')}")
                with st.expander("💡 콘텐츠 아이디어"): st.write(row.get('아이디어', '구상 중'))
...
with tabs[0]:
    ...
    if st.button("💾 구글 시트에 컨펌 결과 저장하기"):
        if conn:
            ...
            except: st.error("구글 시트 쓰기 권한을 확인하세요.")
        else: st.error("구글 시트 연결이 안 되어있습니다.")

with tabs[1]:
    st.header("🏢 자사 크리에이터")
    st.subheader("💎 전속")
    draw_gallery(df_yt_master[(df_yt_master['구분']=='자사') & (df_yt_master['세부유형']=='전속')])
    st.divider()
    st.subheader("🤝 파트너십")
    draw_gallery(df_yt_master[(df_yt_master['구분']=='자사') & (df_yt_master['세부유형']=='파트너십')])
    st.divider()
    st.subheader("🚀 프로젝트 협업")
    draw_gallery(df_yt_master[(df_yt_master['구분']=='자사') & (df_yt_master['세부유형']=='프로젝트 협업')])
...
with tabs[3]:
    ...
    st.dataframe(df_vendor_master, use_container_width=True, hide_index=True, column_config={"URL": st.column_config.LinkColumn("링크")})
